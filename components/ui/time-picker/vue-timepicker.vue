      ...
      this.tokenChunks.forEach(chk => {
        ...
        if (chk.type === "apm" && this.has.customApmText) {
          if (this.apm && this.apm.length) {
            ...
            chunkCurrentLen =
              customApmText && customApmText.length ? customApmText.length : chk.len;
          } else {
          ...
          chunkCurrentLen =
            this[chk.type] && this[chk.type].length ? this[chk.type].length : chk.len;
        }
        list.push({
          token: chk.token,
          type: chk.type,
  ...
  methods: {
    ...
    debounceSetInputSelection({ start = 0, end = 0 }) {
      ...
      this.selectionTimer = window.setTimeout(() => {
        ...
        if (
          this.$refs.input &&
          (this.$refs.input.selectionStart !== start || this.$refs.input.selectionEnd !== end)
        ) {
          this.setInputSelectionRange(start, end);
        }
      }, 30);
...
<template>
  <span class="vue__time-picker time-picker" :style="inputWidthStyle">
    ...
    <div class="controls" v-if="showClearBtn || opts.hideDropdown" tabindex="-1">
      <span class="clear-btn" v-if="!isActive && showClearBtn" @click="clearTime" tabindex="-1"
        >&times;</span
      >
      <span
          ...
          <template v-for="column in columnsSequence">
            <ul v-if="column === 'hour'" :key="column" class="hours" @scroll="keepFocusing">
              ...
              <template v-for="(hr, hIndex) in hours">
                <li
                  v-if="
                    !opts.hideDisabledHours || (opts.hideDisabledHours && !isDisabled('hour', hr))
                  "
                  :key="hIndex"
            ...
            <ul v-if="column === 'apm'" :key="column" class="apms" @scroll="keepFocusing">
              ...
              <template v-for="(a, aIndex) in apms">
                <li
                  v-if="
                    !opts.hideDisabledHours || (opts.hideDisabledHours && !isDisabled('apm', a))
                  "
                  :key="aIndex"
                  :class="{ active: apm === a }"
